...
def search_path(z_p, m_p, logs_p, x_mask, y_mask, mas_noise_scale=0.01):
    with torch.no_grad():
        ...
        logp2 = torch.matmul(-0.5 * (z_p**2).transpose(1, 2), o_scale)  # [b, t', d] x [b, d, t] = [b, t', t]
        logp3 = torch.matmul(z_p.transpose(1, 2), (m_p * o_scale))  # [b, t', d] x [b, d, t] = [b, t', t]
        ...
        attn = maximum_path_cuda(logp, attn_mask.squeeze(1)).unsqueeze(1).detach()  # [b, 1, t', t]
    return attn
