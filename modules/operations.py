    ...
    def forward(self, query, key, value, mask=None, require_w=False):
        ...
        if self.gaussian_bias:
            ...
            if L <= self.bias_matrix.size(0):
                ...
                gaussian_mask = torch.tensor([[-abs(i - j) ** 2 / 2.0 for i in range(L)] for j in range(L)]).repeat(self.n_heads, 1, 1).to(QK.device)
...
class EncGausSALayer(nn.Module):
    def __init__(self, hid_dim, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1, gaus_bias=False, gaus_tao=10):
        super().__init__()
...
class MultiheadAttention(nn.Module):
    ...
    def _get_input_buffer(self, incremental_state):
        return (
            tts_utils.get_incremental_state(
                self,
                incremental_state,
                "attn_state",
            )
    ...
    def forward(self, x, encoder_padding_mask=None, **kwargs):
        ...
        for i in range(0, x.shape[0], half_chunk_size + 1):
            ...
            (
                x_,
                _,
            ) = self.self_attn(query=q, key=kv, value=kv, key_padding_mask=encoder_padding_mask_, attn_mask=attn_mask[i : i + half_chunk_size + 1, k_start:k_end])
...
class ConvAttentionLayer(nn.Module):
    ...
    def forward(self, x, key, value, encoder_padding_mask=None, enc_dec_attn_constraint_mask=None):
        ...
        if encoder_padding_mask is not None:
            attn_weights = attn_weights.masked_fill(encoder_padding_mask.unsqueeze(1), float("-inf")).type_as(attn_weights)  # FP16 support: cast to float and back
...
class DecConvLayer(nn.Module):
    ...
    def forward(self, x, encoder_out=None, encoder_padding_mask=None, incremental_state=None, **kwargs):
        ...
        x, attn = self.attention(
            ...
            static_kv=True,
            enc_dec_attn_constraint_mask=tts_utils.get_incremental_state(self, incremental_state, "enc_dec_attn_constraint_mask"),
        )
        x = F.dropout(x, self.dropout, training=self.training)
        x = residual + x
        attn_logits = attn[1]
...
class LSTMAttentionLayer(nn.Module):
    ...
    def forward(self, input, source_hids, encoder_padding_mask=None, enc_dec_attn_constraint_mask=None):
        ...
        # don't attend over padding
        if encoder_padding_mask is not None:
            attn_weights = attn_weights.float().masked_fill_(encoder_padding_mask.unsqueeze(1), float("-inf")).type_as(attn_weights)  # FP16 support: cast to float and back
...
class DecLSTMLayer(nn.Module):
    ...
    def clear_buffer(self, input, encoder_out=None, encoder_padding_mask=None, incremental_state=None):
        if incremental_state is not None:
            prev_hiddens = encoder_out.mean(dim=0, keepdim=True)
            prev_cells = encoder_out.mean(dim=0, keepdim=True)
            tts_utils.set_incremental_state(self, incremental_state, "cached_state", (prev_hiddens, prev_cells))
...
OPERATIONS_ENCODER = {  # c = hidden size
    ...
    8: lambda c, dropout: EncSALayer(c, 2, dropout=dropout, attention_dropout=0.0, relu_dropout=dropout, kernel_size=hparams["enc_ffn_kernel_size"], padding=hparams["ffn_padding"]),
...
OPERATIONS_DECODER = {
    ...
    8: lambda c, dropout: DecSALayer(c, 2, dropout=dropout, attention_dropout=0.0, relu_dropout=dropout, kernel_size=hparams["dec_ffn_kernel_size"]),
