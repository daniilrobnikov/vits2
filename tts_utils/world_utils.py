...
! Don't install `pysptk` in requirements.txt
import pysptk
import copy
...
def f0_to_coarse(f0: np.ndarray):
...
def f0_to_coarse_torch(f0: torch.Tensor):
...
def process_f0(f0: np.ndarray, hparams):
...
def restore_pitch(pitch: torch.Tensor, uv, hparams, pitch_padding=None, min=None, max=None):
